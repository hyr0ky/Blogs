#区块链 #web3 #bugbounty #oauth #cors #moblie
...
```bash
# 攻击步骤：
1. 目标OAuth配置中白名单包含：http://localhost:3000
2. 攻击者创建恶意Android应用
3. 应用启动本地Web服务器(端口3000)
4. 应用打开浏览器指向Google OAuth页面，redirect_uri=http://localhost:3000
5. 用户授权后，令牌被发送到攻击者的本地服务器
```
...
```bash
# 攻击步骤：
1. 目标GSI配置中白名单包含localhost源
2. 攻击者创建恶意应用，托管伪造登录页面
3. 页面使用auto_select参数自动触发身份验证
4. 利用已保存的会话自动获取令牌
```
...
- Google Oauth ：https://developers.google.com/identity/gsi/web/guides/get-google-api-clientid?hl=zh-cn
- ![](media/Pasted%20image%2020251127025809.png)  
- ![](media/Pasted%20image%2020251127030051.png)  



#### 批量

1. 收集client_id 和redirct_uri ![](media/Pasted%20image%2020251127041931.png)  
2. burp ![](media/Pasted%20image%2020251127041901.png)  